  ...
  useEffect(() => {
    if (id !== "") {
      if (window.confirm("Do you want delete this user")) {
        axios
          .delete(`http://localhost:57678/DeleteSubCategory/${id}`)
          .then((res) => {
  ...
  useEffect(() => {
    if (search === "") {
      ...
      var filterList = props.list.filter((object) => {
        return (object.categoryName.toLowerCase().indexOf(search.toLowerCase()) > -1) || (object.name.toLowerCase().indexOf(search.toLowerCase()) > -1);
      });
...
// Hook action paging 
  ...
  return (
    <div className={styles.cardCategoryList}>
      <div className={styles.categoryListTitle}>SubCategories List</div>
          ...
          {filter.slice(firstIndex, lastIndex)?.map((element, index) => {
            return (
              <div className={styles.itemCategoryList} key={element.id}>
                ...
                <div className={styles.itemCategory}>{element.categoryName}</div>
                ...
                <div className={styles.itemAction}>
                  ...
                  <button className={styles.buttonActionDelete} value={id}
                    onClick={() => {
                      setId(element.id);
                    }}>
