...
{  
    public class GetTotalEnrollmentsByDay : BaseAsyncEndpoint<object>
    {
        private readonly IDashboardService _dashboardService;

        public GetTotalEnrollmentsByDay(IDashboardService dashboardService)
        {
            _dashboardService = dashboardService;
        }


        [HttpGet("/dashboard/get-total-enrollment-by-day")]
        [SwaggerOperation(
        ...
        public override async Task<ActionResult<object>> HandleAsync(CancellationToken cancellationToken)
        {
            ...
            var response = await _dashboardService.GetTotalEnrollmentsByDay(id);
            return Ok(response);
        }
