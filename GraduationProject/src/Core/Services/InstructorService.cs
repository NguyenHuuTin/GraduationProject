...
using Core.Specifications.InstrructorSpec;
using Core.Specifications.InstructorSpec;
...
namespace Core.Services
{
    public class InstructorService : IInstructorService
    {
        ...
        public async Task<Result<User>> UpdateProfileAsync(Guid id, UpdateProfileRequest request)
        {
            if (id == null)
            {
                var errors = new List<ValidationError>();
            ...
            try
            {
                ...
                //check email
                var emailExistedSpec = new EmailExistedSpecification(request.Email);
                ...
                //check username
                var userNameExistedSpec = new EmailExistedSpecification(request.UserName);
                ...
                item.FacebookLink = request.FacebookLink; ;
                item.LinkedLink = request.LinkedLink;
                item.YoutubeLink = request.YoutubeLink ;
                item.LinkedLink = request.LinkedLink;
                item.ProfileLink = request.ProfileLink;
                item.TwitterLink = request.ProfileLink;
